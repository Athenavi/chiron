/**
 * Chiron 会话地图 —— 基于 dsh-synapse 构建
 * =========================================
 * 本文件（以及同目录的 styles.css）移植自 https://github.com/liangmianya/dsh-synapse
 * （DeepSeek Harness 的可视化对话工作台插件 Synapse），保留其全部 UI/UX：
 * 画布 + SVG 连线、卡片尺寸与层级、inspector 详情、选区追问、折叠子树、快捷短语。
 *
 * 本文件**不直接依赖任何 DSH 专有接口**：它只经 `api(...)`（REST）与 `dshRpc(...)`（RPC）
 * 两个入口取数，这两个入口都在 adapter.js 里被映射到 Chiron 的
 * `/v1/session-map`（布局）与 `/v1/conversations`（会话/消息/fork）。
 *
 * 相对上游的改动只有"去 DSH 化"这几处：
 *   1. 移除「在 DSH 中打开」按钮 —— Chiron 里"点卡片即切会话"，
 *      由 postMessage('synapse:activate-session') + ChatView 的监听完成；
...
const CARD_POSITIONS_KEY = 'chiron-map:card-positions:v3'
const COLLAPSED_CARDS_KEY = 'chiron-map:collapsed-cards:v1'
const QUICK_PHRASES_KEY = 'chiron-map:quick-phrases:v1'
...
const savedBranchAnchors = (() => {
  try {
    const value = JSON.parse(localStorage.getItem('chiron-map:branch-anchors') ?? '[]')
...
const savedCardPositions = (() => {
  try {
    ...
    localStorage.removeItem('chiron-map:card-positions:v2')
...
function rememberBranchAnchor(sessionId, cardId) {
  ...
  try { localStorage.setItem('chiron-map:branch-anchors', JSON.stringify([...state.branchAnchors])) } catch { /* Private browsing may disable local storage. */ }
}
...
function post(type, payload = {}) {
  if (window.parent !== window) window.parent.postMessage({ source: 'chiron-sessionmap', type, ...payload }, window.location.origin)
}

function dshRpc(type, payload = {}) {
  if (window.parent === window) return Promise.reject(new Error('会话地图未就绪，请刷新页面重试'))
  const requestId = crypto.randomUUID()
  post(type, { requestId, ...payload })
  return new Promise((resolve, reject) => {
    const timer = window.setTimeout(() => {
      state.pendingRpc.delete(requestId)
      reject(new Error('会话服务未在规定时间内响应'))
    }, 20_000)
...
function openContinue(parent, anchorId = undefined, text = '') {
  if (parent.dshSessionId === null) return setError('该节点没有关联的会话')
...
function conversationCard(card, graph) {
  ...
  const source = card.parentId === null ? '会话' : card.turnIndex === 0 ? '分支' : '追问'
...
function renderCanvas() {
  ...
  if (threads.length === 0 && state.draft?.kind !== 'new') return `<section class="empty-canvas"><strong>会话为空</strong><p>点击新会话，在画布中输入第一条消息。</p><div><button class="primary" type="button" data-action="create-session">新建会话</button></div></section>`
...
function threadMessage(thread, message) {
  ...
  const label = isUser ? '你' : message.kind === 'assistant' ? '助手' : message.kind === 'error' ? '错误' : '记录'
...
// ── 便签双击编辑（移植时补的能力）──
//
// 上游没有"便签"概念；而我们从旧版 localStorage 迁移过来的**手写便签**是
// session_id 为空的无会话节点，双击它应当能改文字（而不是去开一个不存在的会话）。
// 有会话的卡片不受影响：双击仍交给浏览器（选词/选择），避免抢用户的手势。
...
app.addEventListener('click', event => {
  // 强制刷新（手动触发）+ 防抖。
//
// 为什么不做周期刷新：地图不是常看视图，定时拉取纯属浪费系统资源。
// 侧栏按钮手动触发即可 —— 配合后端 60s→10min 的兜底对账，一致性仍然有人管。
let refreshCooldown = 0
function forceRefreshCanvas() {
  // 防抖：冷却期内连点直接忽略，避免重复拉取与重复重渲染
  if (refreshCooldown !== 0) return
  refreshCooldown = window.setTimeout(() => { refreshCooldown = 0 }, 1200)
  void refreshSummaries({ renderAfter: false })
    .then(() => {
      if (state.workspace === null) return undefined
      return openWorkspace(state.workspace.id, { preserveCanvasCamera: true })
    })
    .catch(setError)
  render()
}
...
app.addEventListener('click', event => {
  const btn = event.target instanceof Element ? event.target.closest('[data-action="refresh-canvas"]') : null
  if (!btn) return
  event.preventDefault()
  forceRefreshCanvas()
})
...
const el = event.target instanceof Element ? event.target.closest('[data-action="create-canvas"]') : null
  if (!el) return
  event.preventDefault()
  const name = window.prompt('画布名称', '我的地图')
  if (name === null) return
  void api('/synapse/api/workspaces', {
    method: 'POST',
    body: JSON.stringify({ title: name.trim() || '我的地图' }),
  }).then(() => refreshSummaries()).catch(setError)
})
app.addEventListener('dblclick', event => {
  const el = event.target instanceof Element ? event.target.closest('[data-card-id]') : null
  // 双击**空白处** → 新建便签。上游没有便签概念（这是我们补的能力），所以也没有创建入口；
  // 而旧版迁移过来的便签必须能继续新建，否则这个能力只减不增。
  // 落点：放在现有内容下方一行（相机换算需要上游内部状态，这里取可预测的位置，用户随即拖动即可）。
  if (!(el instanceof HTMLElement)) {
    if (!(event.target instanceof Element) || !event.target.closest('.canvas-viewport')) return
    if (state.workspace === null) return
    const noteTitle = window.prompt('新建便签', '')
    if (noteTitle === null || noteTitle.trim() === '') return
    const noteId = 'note:' + Date.now().toString(36)
    const bottom = state.workspace.threads.reduce((max, item) => Math.max(max, item.position?.y ?? 0), 0)
    ...
    // PATCH 一个未知 id 在 adapter 里等价于"新建节点"
    void api(`/synapse/api/threads/${noteId}`, {
      method: 'PATCH',
  ...
  const card = state.canvasCardsById?.get(el.dataset.cardId ?? '')
  const thread = state.workspace?.threads.find(item => item.id === card?.dshThreadId)
  if (!thread || thread.dshSessionId) return
  const next = window.prompt('便签内容', thread.title ?? '')
  if (next === null) return
  thread.title = next
  void api(`/synapse/api/threads/${thread.id}`, {
    method: 'PATCH',
...
window.addEventListener('message', event => {
  if (event.origin !== window.location.origin || event.data?.source !== 'chiron-sessionmap') return
