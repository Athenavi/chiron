...
| `mypy`（已接线范围，见 L2-1） | 0 —— 47 个路径，见 `.github/workflows/ci.yml` 的 `Mypy (strict)` step |
...
- **已量化**（mypy 2.3.1）：接线前基线 **1171 errors / 130 个文件（共 200 个源文件）**，
  ...
  **截至第十四批已清 620 条 → 551 / 55 文件**。结论：1171 条**不属于需要下调 `strict` 的量级**
  （备选阈值是「数万条」）。
- **实施顺序**：
  1. ~~装 mypy 跑 `mypy app/`，记录错误总数与按域分布~~ —— 已完成，数据见上；
  2. **分批接线**：✅ 已完成 **14 批、清 620 条**，全部零行为变更、引擎套件 `1259 passed` 未变。
     ...
     - 继续方式：每清零一块就往 `.github/workflows/ci.yml` 的 `Mypy (strict)` step 列表里追加
       （现为 **47 个路径**）；`mypy app/` 全绿后删掉 `--follow-imports=silent`；
     - 下一步候选（按文件切；`python -m mypy app/` 的存量，共 551 条 / 55 个文件）：
       `agent/runtime.py` 99、`main.py` 50、`rag/builder.py` 45、`queue/worker.py` 35、
       `skill/manager.py` 31、`api/media.py` 15、`api/workflows.py` 13、`agent/collaboration.py` 13 …；
